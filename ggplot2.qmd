...
Plus haut, je vous ai promis que les graphes `ggplot2` étaient en soit des machines à faire des graphes[^funct]. Imaginons que nous disposions d'un autre jeu de données se présentant de la même façon, typiquement avec les mêmes colonnes utilisées par un `ggplot` que vous auriez construit avec amour et simplicité :

[^funct]: on pourrait parler de fonctions, et même de _functionnals_.
