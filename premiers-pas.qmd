...
R attend que vous tapiez des commandes avec vos jolis doigts et clavier et que vous pressiez `<Entrée>` pour exécuter la commande. Le résultat s'affiche alors :
...
Un concept central est que R n'a pas de mémoire d'un jour à l'autre si vous le fermez. Alors oui, vous pouvez toujours "sauver votre espace de travail" mais je vous le déconseille fortement. La reproductibilité de vos analyses est en jeu. Nous allons apprendre à faire sans.
...
Vous venez d'utiliser - peut-être sans le savoir -, votre première fonction : `sqrt`, pour _square root_. Les fonctions ont souvent des noms explicites et sont, généralement, suivies de parenthèses dans lesquelles glisser une ou des valeurs. 
...
[^fonctions2]: ``  `<-`(toto, 4); toto*2 `` puisqu'on te dit que tout est fonction !
...
Jusqu'ici toutes les variables assignées étaient des scalaires, c'est à dire des variables ne comprenant qu'une valeur. `toto` n'est pas un scalaire mais un _vecteur_ de nombres, qui peut s'écrire en ligne, c'est à dire en une seule et unique _dimension_. Je pose ça là, nous y reviendrons bien vite.
...
Revenons à notre fonction `seq` bien pratique pour créer des séquences régulières. En tapant `?seq` on accède au contenu suivant :
